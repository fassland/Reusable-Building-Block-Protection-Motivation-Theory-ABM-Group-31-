...
# THIS IS ORIGINAL CODE, BUT IS NOT USED
...
def calculate_basic_flood_damage(flood_depth, isadapted):
    """
    ...
    if isadapted == True:
        if flood_depth >= 6:
            flood_damage = 1*0.5*(random.random()/0.5) #average of random number is 0.5, so this gets increased to 1 (by deviding with 0.5 ), then the flood damage gets decreased by 50% with adaptation
        elif flood_depth < 0.025:
            flood_damage = 0
        else:
            # see flood_damage.xlsx for function generation
            flood_damage = (0.1746 * math.log(flood_depth) + 0.6483)*0.25*(random.random()/0.5)
    else:
        if flood_depth >= 6:
            flood_damage = 1
        elif flood_depth < 0.025:
...
# code to implement social media campaign
def people_centered_response(coping,risk,increase, flood_damage_estimated): #social media
    actual_increase = increase * (flood_damage_estimated/0.5) * (random.random()/0.5)
    # social media campaign can target specific appraisals through data analysis
    # we implemented this assumption with targeted increase for the lower appraisal
    if coping > risk:
        coping_increase = 0
        risk_increase = actual_increase
    else: #coping =< risk
        coping_increase = actual_increase
        risk_increase = 0
    
    return coping_increase, risk_increase